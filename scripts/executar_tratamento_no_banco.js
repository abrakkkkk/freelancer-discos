...
async function main() {
  console.log('=== Iniciando Tratamento e Atualização no Supabase ===');
  
  const relatorio = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'relatorio_refinado.json'), 'utf8'));
  console.log(`Total de discos a atualizar: ${relatorio.length}`);

  // Passo 1: Fazer Backup dos registros atuais no banco
  console.log('\n[Passo 1/4] Fazendo backup do estado atual dos 537 registros...');
  const ids = relatorio.map(r => r.id);
  
  let backup = [];
  // Buscar em lotes de 100 por causa da cláusula in
  for (let i = 0; i < ids.length; i += 100) {
    const chunk = ids.slice(i, i + 100);
    const { data, error } = await supabase
      .from('discos')
      .select('id, artista, titulo, atualizado_em, caixa, loja')
      .in('id', chunk);
  ...
  const backupPath = path.resolve(__dirname, 'backup_tratamento_dados_pre_update.json');
  fs.writeFileSync(backupPath, JSON.stringify(backup, null, 2), 'utf8');
  console.log(`Backup salvo com sucesso (${backup.length} registros) em: ${backupPath}`);

  // Passo 2: Executar as atualizações no Supabase
  console.log('\n[Passo 2/4] Executando updates no banco...');
  let sucessoCount = 0;
  let erroCount = 0;
  const erros = [];

  // Concorrência moderada (pool de 10) para velocidade com estabilidade
  const poolSize = 10;
  for (let i = 0; i < relatorio.length; i += poolSize) {
    ...
    await Promise.all(slice.map(async (item) => {
      ...
      const { error } = await supabase
        .from('discos')
        .update({
          artista: item.artistaRecomendado,
          titulo: item.tituloRecomendado,
          atualizado_em: now
        })
        .eq('id', item.id);
  ...
  // Passo 3: Invalidação de Cache de Capas (conforme regra de arquitetura)
  console.log('\n[Passo 4/4] Limpando/Invalidando cache de capas para os discos alterados...');
  ...
  if (fs.existsSync(coversPath)) {
    ...
    if (coversRemovidas > 0) {
      ...
      console.log(`Foram invalidadas ${coversRemovidas} capas em src/data/covers_cache.json para sincronização limpa.`);
    } else {
