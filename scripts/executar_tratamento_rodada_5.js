...
async function main() {
  console.log('=== Iniciando Execução da 5ª Rodada (Top 130 Discos Mais Caros - Preços e Anos) ===');
  
  const relatorio = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'relatorio_anos_top_130.json'), 'utf8'));
  ...
  const backupPath = path.resolve(__dirname, 'backup_tratamento_dados_rodada_5_pre_update.json');
  ...
  console.log('\n=== 5ª Rodada concluída com sucesso! ===');
}
