...
async function main() {
  console.log('=== Iniciando Execução da 2ª Rodada de Tratamento no Supabase ===');
  
  const relatorio = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'relatorio_rodada_2.json'), 'utf8'));
  console.log(`Total de discos a atualizar: ${relatorio.length}`);

  // Passo 1: Backup
  console.log('\n[Passo 1/4] Fazendo backup do estado atual dos 211 registros...');
  ...
  const backupPath = path.resolve(__dirname, 'backup_tratamento_dados_rodada_2_pre_update.json');
  ...
  console.log('\n[Passo 4/4] Limpando/Invalidando cache de capas para discos da 2ª rodada...');
  ...
  console.log('\n=== 2ª Rodada concluída com sucesso! ===');
}
